...
- `watchFile`：监听单个文件的策略，它可以有以下值
  ...
  - `priorityPollingInterval`，以固定的时间间隔，检查文件的更改，但是使用「试探法」（heuristics）检查某些类型的文件的频率比其他文件低。（heuristics 概念可参考：https://stackoverflow.com/questions/8445638/concrete-examples-of-heuristics）
