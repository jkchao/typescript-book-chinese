...
编译上下文算是一个比较花哨的术语，它用来给文件分组，TypeScript 将会解析它，哪些是有效的，哪些是无效的。除了所需要编译的文件所携带信息外，编译上下文也包含了有哪些编译选项正在使用。定义逻辑分组，一个比较好的方式是使用 `tsconfig.json` 文件。
...
你甚至可以使用 `tsc -w` 来启用 TypeScript 编译器的观测模式，在文件改动之后，它将重新编译。
...
或者，你可以使用 `include` 和 `exclude` 选项来指定需要包含的文件，和排出的文件：
